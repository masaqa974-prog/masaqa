...
// ADMIN MENU
function zet_menu($title = NULL) {
  echo '<link href="' . osc_base_url() . 'oc-content/themes/zeta/admin/css/admin.css?v=' . date('YmdHis') . '" rel="stylesheet" type="text/css" />';
  echo '<link href="' . osc_base_url() . 'oc-content/themes/zeta/admin/css/bootstrap-switch.css" rel="stylesheet" type="text/css" />';
  echo '<link href="' . osc_base_url() . 'oc-content/themes/zeta/admin/css/tipped.css" rel="stylesheet" type="text/css" />';
//  echo '<link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css" rel="stylesheet" type="text/css" />';
  ...
  echo '<script src="' . osc_base_url() . 'oc-content/themes/zeta/admin/js/admin.js?v=' . date('YmdHis') . '"></script>';
  echo '<script src="' . osc_base_url() . 'oc-content/themes/zeta/admin/js/tipped.js"></script>';
  echo '<script src="' . osc_base_url() . 'oc-content/themes/zeta/admin/js/bootstrap-switch.js"></script>';


  $current = basename(Params::getParam('file'));

  $links = array();
  $links[] = array('file' => 'configure.php', 'icon' => 'fa-wrench', 'title' => __('Configure', 'zeta'));
  $links[] = array('file' => 'banner.php', 'icon' => 'fa-clone', 'title' => __('Advertisement', 'zeta'));
  $links[] = array('file' => 'category.php', 'icon' => 'fa-cogs', 'title' => __('Category Icons', 'zeta'));
  $links[] = array('file' => 'logo.php', 'icon' => 'fa-desktop', 'title' => __('Header Logo', 'zeta'));
  $links[] = array('file' => 'plugins.php', 'icon' => 'fa-puzzle-piece', 'title' => __('Plugins', 'zeta'));

  if($title == '') { $title = __('Configure', 'zeta'); }

  $text  = '<div class="mb-head">';
  $text .= '<div class="mb-head-left">';
  $text .= '<h1>' . $title . '</h1>';
  $text .= '<h2>Zeta Osclass Theme</h2>';
  $text .= '</div>';
  $text .= '<div class="mb-head-right">';
  $text .= '<ul class="mb-menu">';

  foreach($links as $l) {
    $text .= '<li><a href="' . osc_admin_base_url(true) . '?page=appearance&action=render&file=oc-content/themes/zeta/admin/' . $l['file'] . '" class="' . ($l['file'] == $current ? 'active' : '') . '"><i class="fa ' . $l['icon'] . '"></i><span>' . $l['title'] . '</span></a></li>';
  }
...
// List of categories
function zet_has_subcategories_special($categories, $deep = 0) {
  ...
  foreach($categories as $c) {
    $extra = ModelZET::newInstance()->getCategoryExtra($c['pk_i_id']);
    ?>
    ...
    <div class="mb-table-row <?php echo ($deep == 0 ? 'parent' . ' o' . $i : ''); ?>">
      ...
      <div class="mb-col-3 mb-align-left icon">
        ...
        <?php } else if(zet_param('sample_images') == 1 && file_exists($upload_dir_small_sample . $c['pk_i_id'] . '.png')) { ?>
      ...
      <div class="mb-col-3 mb-align-left">
        <a class="add_img" id="category[<?php echo $c['pk_i_id']; ?>][icon]" href="#"><?php _e('Upload icon', 'zeta'); ?></a>
      </div>
      ...
      <div class="mb-col-3 mb-align-left">
        <a class="add_fa" id="category[<?php echo $c['pk_i_id']; ?>][faicon]" href="#" title="To remove icon click on link and leave input empty."><?php _e('Add icon', 'zeta'); ?></a>
      </div>
      ...
      <div class="mb-col-3 mb-align-left">
        <a class="add_color" id="category[<?php echo $c['pk_i_id']; ?>][color]" href="#" title="To remove color click on link and leave input empty."><?php _e('Add color', 'zeta'); ?></a>
      </div>
    </div>

    <?php
    if(isset($c['categories']) && is_array($c['categories']) && !empty($c['categories'])) {
      zet_has_subcategories_special($c['categories'], $deep+1);
    }   
