<?php
  ...
  $user = osc_user();
  $location_type = zet_param('profile_location');
...
<body id="user-profile" class="body-ua <?php osc_run_hook('body_class'); ?>">
  <?php 
  ...
  <div class="container primary">
    ...
    <div id="user-main">
        ...
        <form action="<?php echo osc_base_url(true); ?>" method="post" class="profile">
          <div class="wrap">
            <div class="left-block">
              ...
              <?php if(osc_profile_img_users_enabled()) { ?>
                <div class="control-group profile-img">
                  <label class="control-label" for="name"><?php _e('Profile picture (avatar)', 'zeta'); ?></label>
                  <div class="controls">
                    <div class="user-img">
              ...
              <div class="row">
                <label for="name"><?php _e('Your name', 'zeta'); ?> <span class="req">*</span></label>
                ...
                <?php if(function_exists('profile_picture_show') && !osc_profile_img_users_enabled()) { ?>
                  <a href="#" class="update-avatar"><?php _e('Update avatar', 'zeta'); ?></a>
              ...
              <div class="row">
                <label for="email"><?php _e('E-mail', 'zeta'); ?> <a href="#" class="change-email"><?php _e('Edit', 'zeta'); ?></a></label>
                <div class="input-box"><input type="text" disabled value="<?php echo osc_esc_html(osc_user_email()); ?>"/></div>
              </div>

              <div class="row">
                <label for="phoneMobile"><?php _e('Mobile phone', 'zeta'); ?> <span class="req">*</span></label>
                <div class="input-box"><?php UserForm::mobile_text(osc_user()); ?></div>
              </div>

              <div class="row">
                <label for="phoneLand"><?php _e('Land Phone', 'zeta'); ?></label>
                <div class="input-box"><?php UserForm::phone_land_text(osc_user()); ?></div>
              </div>                        

              <div class="row">
                <label for="webSite"><?php _e('Website', 'zeta'); ?></label>
                <div class="input-box"><?php UserForm::website_text(osc_user()); ?></div>
              </div>
              
              <div class="row">
                <label for="user_type"><?php _e('User type', 'zeta'); ?></label>
                <div class="input-box"><?php UserForm::is_company_select(osc_user(), __('Personal', 'zeta'), __('Company', 'zeta')); ?></div>
              </div>

              <div class="hooksrow"><?php osc_run_hook('user_form'); ?></div>

              <div class="row">
                <label for="info"><?php _e('About you', 'zeta'); ?></label>
            ...
            <div id="user-loc" class="right-block navigator-fill-selects">
              ...
              <h2><?php _e('Location information', 'zeta'); ?></h2>
              ...
              <?php if(zet_param('default_location') == 1) { ?>
                <div class="row navigator">
                  <a href="#" class="locate-me">
                    ...
                    <strong><?php _e('Use current location', 'zeta'); ?></strong>
                    <span class="status">
              ...
              <div class="row">
                <label for="cityArea"><?php _e('City Area', 'zeta'); ?></label>
              ...
              <div class="row">
                <label for="address"><?php _e('ZIP', 'zeta'); ?></label>
          ...
          <div class="row user-buttons">
            <button type="submit" class="btn btn-primary mbBg"><?php _e('Update', 'zeta'); ?></button>

            <?php if(!zet_is_demo()) { ?>
              <a class="btn-remove-account btn btn-white" href="<?php echo osc_base_url(true).'?page=user&action=delete&id='.osc_user_id().'&secret='.$user['s_secret']; ?>" onclick="return confirm('<?php echo osc_esc_js(__('Are you sure you want to delete your account? This action cannot be undone', 'zeta')); ?>?')"><span><?php _e('Delete account', 'zeta'); ?></span></a>
            <?php } ?>
          </div>
        </form>
      </div>
      ...
      <div class="profile-box alt change-mail">
        <h2><?php _e('Change your email', 'zeta'); ?></h2>
        ...
        <form action="<?php echo osc_base_url(true); ?>" method="post" id="user_email_change" class="user-change">
          ...
          <div class="row">
            <label for="email"><?php _e('Current e-mail', 'zeta'); ?></label>
            <div class="input-box"><input type="text" disabled value="<?php echo osc_esc_html(osc_logged_user_email()); ?>"/></div>
          </div>

          <div class="row">
            <label for="new_email"><?php _e('New e-mail', 'zeta'); ?> <span class="req">*</span></label>
            <div class="input-box"><input type="text" name="new_email" id="new_email" value="" /></div>
          </div>

          <div class="row user-buttons">
            <?php if(zet_is_demo()) { ?>
              <a class="btn mbBg disabled" onclick="return false;" title="<?php echo osc_esc_html(__('You cannot do this on demo site', 'zeta')); ?>"><?php _e('Submit', 'zeta'); ?></a>
            <?php } else { ?>
              <button type="submit" class="btn mbBg" disabled><?php _e('Submit', 'zeta'); ?></button>
            <?php } ?>
          </div>
        </form>
      </div>
      ...
      <div class="profile-box alt change-pass">
        <h2><?php _e('Change your password', 'zeta'); ?></h2>
        ...
        <form action="<?php echo osc_base_url(true); ?>" method="post" id="user_password_change" class="user-change">
          ...
          <div class="row">
            <label for="password"><?php _e('Current password', 'zeta'); ?> <span class="req">*</span></label>
            <div class="input-box"><input type="password" name="password" id="password" value="" /></div>
          </div>

          <div class="row">
            <label for="new_password"><?php _e('New password', 'zeta'); ?> <span class="req">*</span></label>
            <div class="input-box">
          ...
          <div class="row">
            <label for="new_password2"><?php _e('Repeat new password', 'zeta'); ?> <span class="req">*</span></label>
            <div class="input-box">
