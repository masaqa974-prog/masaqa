...
public function getTable_item_zeta() {
  return DB_TABLE_PREFIX.'t_item_zeta';
}
...
public function getTable_category_stats() {
  return DB_TABLE_PREFIX.'t_category_stats';
}


public function getTable_category_zeta() {
  return DB_TABLE_PREFIX.'t_category_zeta';
}
...
public function getTable_favorite_items() {
...
public function import($file) {
  ...
  if(!$this->dao->importSQL($sql)){
    throw new Exception("Error importSQL::ModelZET<br>" . $file . "<br>" . $this->dao->getErrorLevel() . " - " . $this->dao->getErrorDesc());
  }
}


public function install() {
  $this->import('oc-content/themes/zeta/model/struct.sql');
}
...
public function getItemExtra($item_id) {
  ...
  $key = md5(osc_base_url().'ModelZET:getItemExtra:'.$item_id);
  $found = null;
  $cache = osc_cache_get($key, $found);
  
  if($cache === false) {
    $this->dao->select();
    $this->dao->from($this->getTable_item_zeta());
...
public function findClosestCity($lat, $lon) {
  ...
  if($result) {
    ...
    if(isset($data['fk_i_city_id']) && $data['fk_i_city_id'] > 0) {
      $data['d_distance_precise'] = zet_calc_cord_distance($lat, $lon, $data['d_coord_lat'], $data['d_coord_long']);
    }
...
function getPopularCategories($limit = 6, $min = 0) {
  $this->dao->select('c.pk_i_id, c.fk_i_parent_id, d.s_name, p.s_name as s_name_parent');
  $this->dao->from($this->getTable_category() . ' as c');
  $this->dao->join($this->getTable_category_stats() . ' as s', 'c.pk_i_id = s.fk_i_category_id', 'INNER');
  ...
  $this->dao->join($this->getTable_category_description() . ' as p', '(c.fk_i_parent_id = p.fk_i_category_id AND p.fk_c_locale_code = "' . osc_current_user_locale() . '")', 'INNER');

  $this->dao->where('c.b_enabled', 1);
  $this->dao->where('c.fk_i_parent_id is not null');
  $this->dao->where('s.i_num_items >= ' . $min);
  $this->dao->orderBy('i_num_items', 'DESC');
...
public function insertItemExtra($data) {
  return $this->dao->insert($this->getTable_item_zeta(), $data);
}

// UPDATE ITEM EXTRA
public function updateItemExtra($id, $data) {
  return $this->dao->update($this->getTable_item_zeta(), $data, array('fk_i_item_id' => $id));
}

// REPLACE ITEM EXTRA
public function replaceItemExtra($data) {
  return $this->dao->replace($this->getTable_item_zeta(), $data);
}
...
public function getCategoryExtra($category_id) {
  ...
  $key = md5(osc_base_url().'ModelZET:getCategoryExtra:'.$category_id);
  $found = null;
  $cache = osc_cache_get($key, $found);
  
  if($cache === false) {
    $this->dao->select();
    $this->dao->from($this->getTable_category_zeta());
...
public function insertCategoryExtra($data) {
  return $this->dao->insert($this->getTable_category_zeta(), $data);
}

// UPDATE ITEM EXTRA
public function updateCategoryExtra($id, $data) {
  return $this->dao->update($this->getTable_category_zeta(), $data, array('fk_i_category_id' => $id));
}

// REPLACE ITEM EXTRA
public function replaceCategoryExtra($data) {
  return $this->dao->replace($this->getTable_category_zeta(), $data);
}
...
// GET COUNTS HOW MANY PEOPLE LIKED ITEM
public function countItemFavorited($item_id) {
  if($item_id <= 0) { return 0; }

  $this->dao->select('count(*) as i_count');
  $this->dao->from($this->getTable_favorite_items());
  ...
  if($result) {
    $data = $result->row();
    return isset($data['i_count']) ? (int)$data['i_count'] : 0;
  }
...
// GET LOCATIONS IN RADIUS FROM ITEM LOCATION TABLE
public function getLocationsInRadius($lat, $lng, $rad, $source = 'city') {
  if($source == 'city') {
    $this->dao->select('DISTINCT fk_i_city_id');
    
  } else if ($source == 'region') {
    $this->dao->select('DISTINCT fk_i_region_id');
  }
