...
<body id="user-alerts" class="body-ua <?php osc_run_hook('body_class'); ?>">
  ...
  <?php
    ...
    while(osc_has_alerts()) { 
      ...
      $search_details['extraTables'] = zet_base64url_encode(json_encode($alert_details['no_catched_tables']));
      $search_details['extraConditions'] = zet_base64url_encode(json_encode($alert_details['no_catched_conditions']));
      ...
      $name = sprintf(__('%s items in %s', 'zeta'), ($cat <> '' ? $cat : __('All categories', 'zeta')), ($loc <> '' ? $loc : __('all locations', 'zeta')));
  ...
  <div class="container primary">
    ...
    <div id="user-main">
      ...
      <h1><?php _e('Saved searches', 'zeta'); ?></h1>
      <h2><?php _e('Receive email notification when new item, that match your search criteria, is published.', 'zeta'); ?></h2>
      ...
      <div class="alerts-box">
        <?php if(is_array($alerts) && count($alerts) > 0) { ?>
            ...
            <div class="alert">
              <div class="head-row">
                <div class="data">
                  ...
                  <div>
                    <span><?php echo sprintf(__('%s listings match subscription conditions, %s notifications, created on %s, id: #%s. Your listings are excluded.', 'zeta'), $a['count'], zet_alert_frequency($a['e_type']), date('j. M Y', strtotime($a['dt_date'])), $a['pk_i_id']); ?></span>
                    <a href="#" class="show-technical-details"><?php _e('Technical details', 'zeta'); ?></a>  
                  </div>
                </div>
                
                <a href="<?php echo $a['unsubscribe_url']; ?>" class="btn btn-secondary mini" onclick="javascript:return confirm('<?php echo osc_esc_js(__('This action can\'t be undone. Are you sure you want to continue?', 'zeta')); ?>');"><?php _e('Delete', 'zeta'); ?></a>
              </div>

              <div class="details"><?php _e('Technical details', 'zeta'); ?>:<br/><?php echo $a['s_search']; ?></div>
              
              <div class="items">
                <a href="<?php echo $a['search_url']; ?>" class="show-all"><?php _e('View all matching items', 'zeta'); ?> &#8594;</a>

                <?php if(osc_count_items() <= 0) { ?>
                  <div class="empty"><?php _e('No items match alert conditions. You will receive email notification once there is new listing published matching alert conditions.', 'zeta'); ?></div>
                <?php } else { ?>
                  <div id="alert-items" class="nice-scroll-wrap nice-scroll-have-overflow">
                    <div class="nice-scroll-prev"><?php echo ICON_SCROLL_PREV; ?></div>
                    
                    <div class="products grid nice-scroll style3">
                      <?php
                        $c = 1; 
                        
                        while(osc_has_items()) {
                          zet_draw_item($c, false, 'tall');
                          $c++;
                        }
                      ?>
                    </div>
          ...
          <?php } ?>
        <?php } else { ?>
          <div class="empty"><?php _e('No alerts has been created yet.', 'zeta'); ?></div>
        <?php } ?>
      </div>
    </div>
  </div>
